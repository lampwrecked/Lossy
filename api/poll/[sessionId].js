...
async function mintNft(session, buyerWallet) {
  ...
  const nftStorageData = await nftStorageRes.json();
  if (!nftStorageData.ok) throw new Error('NFT.Storage upload failed: ' + JSON.stringify(nftStorageData));
  const metadataUri = `https://ipfs.io/ipfs/${nftStorageData.value.cid}`;
